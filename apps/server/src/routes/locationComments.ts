...
const locationCommentsRoute = new HonoVar()
  .basePath('/location-comments')
  .delete('/:id', isAuth(), sValidator('param', type({ id: 'string' })), async (ctx) => {
    ...
    let deleteQuery = db.deleteFrom('location_comments').where('id', '=', id)
...
export default locationCommentsRoute
