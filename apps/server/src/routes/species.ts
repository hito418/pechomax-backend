            ...
            .select((eb2) => [
              jsonObjectFrom(
                eb2
                  ...
                  .selectAll()
                  .whereRef('locations.id', '=', 'speciesLocation.location_id')
              ).as('location'),
            ])
            .whereRef('speciesLocation.species_id', '=', 'species.id')
        ).as('speciesLoactions'),
      ])
...
speciesRoute.post(
  ...
  async (ctx) => {
    ...
    const returningSpecies = await db
      .selectFrom('species')
      .selectAll('species')
      .select((eb) => [
        jsonArrayFrom(
      ...
      .where('species.id', '=', speciesItem.id)
      .executeTakeFirst()

    return ctx.json(returningSpecies, 201)
  }
)
